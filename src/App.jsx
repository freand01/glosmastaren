...
export default function App() {
  ...
  useEffect(() => {
    const loadInitialData = () => {
      ...
      if (listData) {
        try {
          const decodedString = decodeURIComponent(atob(listData));
          ...
          alert("Länken verkar vara trasig. Laddar senast sparade ord istället.");
        }
      }
  ...
  const Navigation = () => (
    <nav className="bg-white/90 backdrop-blur-md shadow-lg shadow-indigo-100/50 border-4 border-white px-6 py-4 flex flex-col md:flex-row justify-between items-center rounded-3xl md:rounded-full mb-10 mt-6 gap-4 z-10 relative">
      ...
      >
        ...
        <h1 className="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-br from-indigo-600 to-purple-600 tracking-tight">
          Glosmästaren
        </h1>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-center gap-3 w-full md:w-auto">
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-indigo-100 text-indigo-700 border-2 border-indigo-200 px-4 py-3 rounded-2xl font-bold hover:bg-indigo-200 hover:border-indigo-300 transition-all shadow-sm active:scale-95"
          title="Vänd håll på översättningen"
        >
        ...
        <button 
          onClick={() => setCurrentView('dashboard')}
          className={`px-5 py-3 rounded-2xl font-black transition-all ${currentView === 'dashboard' ? 'bg-indigo-500 text-white shadow-[0_4px_0_0_#312e81] translate-y-[-2px]' : 'bg-transparent text-slate-500 hover:bg-slate-100'}`}
        >
          Spela
        </button>
        <button 
          onClick={() => setCurrentView('add')}
          className={`px-5 py-3 rounded-2xl font-black transition-all ${currentView === 'add' ? 'bg-indigo-500 text-white shadow-[0_4px_0_0_#312e81] translate-y-[-2px]' : 'bg-transparent text-slate-500 hover:bg-slate-100'}`}
        >
          Gloslista
        </button>
        <button 
          onClick={() => setCurrentView('share')}
          className={`px-5 py-3 rounded-2xl font-black transition-all ${currentView === 'share' ? 'bg-indigo-500 text-white shadow-[0_4px_0_0_#312e81] translate-y-[-2px]' : 'bg-transparent text-slate-500 hover:bg-slate-100'}`}
        >
          Dela/Spara
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 font-sans text-slate-800 pb-16">
  ...
  return (
    <div>
      <header className="mb-12 text-center mt-8">
        <div className="inline-flex items-center justify-center p-3 bg-yellow-100 text-yellow-600 rounded-2xl mb-4 transform -rotate-6 shadow-sm border-2 border-yellow-200">
           <Sparkles size={28} />
        </div>
        <h2 className="text-5xl font-black text-slate-800 mb-4 tracking-tight">Vad vill du spela?</h2>
        <p className="text-xl text-slate-600 font-medium">
          Du har <span className="inline-block bg-indigo-100 text-indigo-700 px-3 py-1 rounded-xl font-black border-2 border-indigo-200 transform rotate-2">{wordsCount} glosor</span> redo att bemästras!
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 px-2">
        {modes.map((mode) => (
          <button
            ...
            className={`group flex flex-col relative w-full p-6 rounded-[2rem] text-white border-b-[8px] transition-all duration-200 hover:-translate-y-2 hover:border-b-[12px] active:translate-y-[4px] active:border-b-[4px] text-left shadow-xl outline-none ${mode.theme}`}
          >
            <div className="bg-white/20 p-4 rounded-2xl self-start mb-6 backdrop-blur-sm">
              {mode.icon}
            </div>
            ...
            {/* Liten dekorativ blob i bakgrunden */}
...
function ManageWords({ words, setWords, ttsLanguage, setTtsLanguage }) {
  ...
  const handleAdd = (e) => {
    ...
    const newWord = { id: Date.now(), term: term.trim(), translation: translation.trim() };
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white p-6 sm:p-10 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border-4 border-white animate-in fade-in">
      ...
      {/* Rubrik & Språkväljare i samma rad */}
        ...
        <div className="relative w-full sm:w-auto bg-slate-50 p-3 rounded-3xl border-2 border-slate-100">
          ...
          <div className="relative">
            <select 
              ...
              className="appearance-none w-full bg-white border-2 border-slate-200 text-slate-700 font-bold rounded-2xl px-4 py-3 pr-10 focus:ring-4 focus:ring-indigo-200 focus:border-indigo-500 outline-none transition-all cursor-pointer shadow-sm hover:bg-slate-50"
            >
      ...
      <form onSubmit={handleAdd} className="flex flex-col sm:flex-row gap-4 mb-10 bg-slate-100 p-4 rounded-[2rem] border-2 border-slate-200/60 shadow-inner">
        <div className="flex-1">
          <input type="text" value={term} onChange={(e) => setTerm(e.target.value)} className="w-full p-4 rounded-2xl border-4 border-white shadow-sm focus:border-indigo-400 focus:ring-0 outline-none font-bold text-lg text-slate-700 bg-white placeholder-slate-400 transition-colors" placeholder="Glosan (t.ex. Cat)" />
        </div>
        <div className="flex-1">
          <input type="text" value={translation} onChange={(e) => setTranslation(e.target.value)} className="w-full p-4 rounded-2xl border-4 border-white shadow-sm focus:border-indigo-400 focus:ring-0 outline-none font-bold text-lg text-slate-700 bg-white placeholder-slate-400 transition-colors" placeholder="Svenska (t.ex. Katt)" />
        </div>
        <div className="flex items-end">
          <button type="submit" className="w-full sm:w-auto h-full bg-indigo-500 hover:bg-indigo-400 text-white px-8 py-4 rounded-2xl border-b-[6px] border-indigo-700 active:border-b-0 active:translate-y-[6px] transition-all font-black text-lg flex justify-center items-center gap-2">
            <PlusCircle size={24} /> <span className="sm:hidden">Lägg till</span>
          </button>
        </div>
      </form>
      ...
      <div className="space-y-4">
        {words.length === 0 ? (
          <div className="text-center py-12 bg-slate-50 rounded-[2rem] border-4 border-dashed border-slate-200 text-slate-400 font-bold text-xl">Inga glosor än! Börja skriva ovan. 👆</div>
        ) : (
...
function ShareSaveView({ words, setWords, ttsLanguage, setTtsLanguage }) {
  ...
  const generateShareLink = () => {
    ...
    const finalUrl = `${baseUrl}?list=${encodedPayload}`;

    navigator.clipboard.writeText(finalUrl).then(() => {
      setCopied(true); setTimeout(() => setCopied(false), 3000);
    }).catch(() => prompt("Kopiera denna länk:", finalUrl));
  };
  ...
  const downloadJSON = () => {
    ...
    dlAnchorElem.setAttribute("href", dataStr); dlAnchorElem.setAttribute("download", "min_gloslista.json");
    dlAnchorElem.click();
  };
    ...
    reader.onload = (event) => {
      try {
        ...
        if (result.words && Array.isArray(result.words)) {
          ...
          if (result.lang) setTtsLanguage(result.lang);
          alert("Gloslistan laddades in!");
    ...
    <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in">
      ...
      <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-8 sm:p-10 rounded-[2.5rem] shadow-xl text-white border-b-[8px] border-indigo-800">
        <div className="flex items-center gap-4 mb-6">
          <div className="p-4 bg-white/20 rounded-2xl"><LinkIcon size={40} /></div>
          <h3 className="text-3xl font-black tracking-tight">Dela listan</h3>
        </div>
        <p className="text-indigo-100 font-medium mb-10 text-lg leading-relaxed">
          Skapa en magisk länk. Eleverna klickar på länken och börjar spela direkt — ingen inloggning krävs!
        </p>
        <button onClick={generateShareLink} className={`w-full font-black py-6 rounded-2xl text-xl flex items-center justify-center gap-3 transition-all border-b-[6px] active:border-b-0 active:translate-y-[6px] ${copied ? 'bg-green-400 text-white border-green-600' : 'bg-white text-indigo-600 border-indigo-200 hover:bg-indigo-50'}`}>
          {copied ? <Check size={28} /> : <Copy size={28} />}
          {copied ? "Länken är kopierad!" : "Skapa & Kopiera Länk"}
        </button>
      </div>
      ...
      <div className="bg-white p-8 sm:p-10 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border-4 border-white">
        <div className="flex items-center gap-4 mb-6">
          <div className="p-4 bg-slate-100 text-slate-500 rounded-2xl"><Download size={40} /></div>
          <h3 className="text-3xl font-black text-slate-800 tracking-tight">Filhantering</h3>
        </div>
        <p className="text-slate-500 font-medium mb-10 text-lg leading-relaxed">
          Spara en backup av glosorna på din dator, eller öppna en fil du sparat tidigare.
        </p>
        
        <div className="flex flex-col gap-4">
          <button onClick={downloadJSON} className="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 border-b-4 border-slate-300 font-bold py-5 rounded-2xl transition-all active:border-b-0 active:translate-y-[4px] flex items-center justify-center gap-3 text-lg">
            <Download size={24} /> Ladda ner fil (.json)
          </button>
          <button onClick={() => fileInputRef.current?.click()} className="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 border-b-4 border-slate-300 font-bold py-5 rounded-2xl transition-all active:border-b-0 active:translate-y-[4px] flex items-center justify-center gap-3 text-lg">
            <Upload size={24} /> Öppna fil från datorn
          </button>
...
function Flashcards({ words, onBack, ttsLanguage, direction }) {
  ...
  return (
    <div className="max-w-3xl mx-auto flex flex-col items-center">
      ...
      <div className="w-full h-[24rem] perspective-1000 cursor-pointer mb-10 group" onClick={() => setIsFlipped(!isFlipped)}>
        <div className={`w-full h-full relative preserve-3d transition-transform duration-500 ease-out ${isFlipped ? 'rotate-y-180' : ''}`}>
          <div className="absolute w-full h-full backface-hidden bg-white border-8 border-blue-50 rounded-[3rem] shadow-2xl flex flex-col items-center justify-center p-10 group-hover:scale-[1.02] transition-transform">
          ...
          <div className="absolute w-full h-full backface-hidden bg-gradient-to-br from-blue-500 to-indigo-600 rounded-[3rem] shadow-2xl flex flex-col items-center justify-center p-10 rotate-y-180 border-8 border-blue-400 text-white group-hover:scale-[1.02] transition-transform">
      ...
      <div className="flex gap-6 w-full max-w-sm justify-between">
        <button onClick={handlePrev} className="bg-white text-indigo-600 flex-1 flex justify-center p-5 rounded-2xl shadow-sm border-b-[6px] border-slate-200 hover:bg-slate-50 active:border-b-0 active:translate-y-[6px] transition-all"><ChevronLeft size={36} /></button>
        <button onClick={handleNext} className="bg-indigo-500 text-white flex-1 flex justify-center p-5 rounded-2xl shadow-sm border-b-[6px] border-indigo-700 hover:bg-indigo-400 active:border-b-0 active:translate-y-[6px] transition-all"><ChevronRight size={36} /></button>
      </div>
    </div>
  );
}
...
function QuizMode({ words, onBack, direction }) {
  ...
  const handleSelect = (option) => {
    ...
    setTimeout(() => {
      if (currentIndex + 1 < words.length) setCurrentIndex(c => c + 1);
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="bg-white p-8 sm:p-12 rounded-[3rem] shadow-xl shadow-purple-100/50 border-4 border-white text-center relative overflow-hidden">
        <div className="absolute top-0 left-0 h-3 bg-slate-100 w-full">
          <div className="h-full bg-purple-500 transition-all duration-500 rounded-r-full" style={{ width: `${(currentIndex / words.length) * 100}%` }}></div>
        </div>

        <div className="bg-purple-50 py-10 rounded-[2rem] border-4 border-purple-100 mb-10 mt-4">
           <p className="text-purple-400 font-bold uppercase tracking-widest text-sm mb-2">Vad betyder</p>
           <h2 className="text-5xl sm:text-6xl font-black text-purple-900">{sides.front}</h2>
        </div>
...
function TypingMode({ words, onBack, direction }) {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      if (currentIndex + 1 < words.length) setCurrentIndex(c => c + 1); else setIsFinished(true);
    }, 2000);
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white p-8 sm:p-12 rounded-[3rem] shadow-xl shadow-orange-100/50 border-4 border-white text-center">
        ...
        <div className="py-12 bg-orange-50 rounded-[2rem] border-4 border-orange-100 mb-10">
          <h2 className="text-5xl sm:text-6xl font-black text-orange-900">{sides.front}</h2>
        </div>
        <form onSubmit={handleSubmit} className="relative mt-8">
          ...
          {!feedback ? (
            <button type="submit" className="mt-8 w-full bg-orange-500 hover:bg-orange-400 text-white font-black py-5 rounded-2xl shadow-sm border-b-[8px] border-orange-700 active:border-b-0 active:translate-y-[8px] transition-all text-2xl uppercase tracking-wider">Rätta</button>
          ) : (
            <div className={`mt-8 p-6 rounded-[2rem] text-2xl font-black border-4 animate-in zoom-in ${feedback === 'correct' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
              {feedback === 'correct' ? 'Snyggt!' : `Rätt svar: ${sides.back}`}
            </div>
          )}
        </form>
      </div>
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white p-8 sm:p-12 rounded-[3rem] shadow-xl shadow-cyan-100/50 border-4 border-white text-center">
        <div className="mb-10 mt-4">
          <button onClick={() => playAudio(sides.front, sides.isFrontForeign ? ttsLanguage : 'sv-SE')} className="w-40 h-40 mx-auto bg-cyan-500 hover:bg-cyan-400 text-white rounded-full flex items-center justify-center shadow-lg border-b-[8px] border-cyan-700 active:border-b-0 active:translate-y-[8px] transition-all group">
            <Volume2 size={80} className="group-hover:scale-110 transition-transform" />
          </button>
          <p className="mt-8 text-cyan-600 font-black uppercase tracking-widest text-lg">Klicka för att lyssna</p>
        </div>
        <form onSubmit={handleSubmit} className="relative mt-8">
          ...
          {!feedback ? (
             <button type="submit" className="mt-8 w-full bg-cyan-500 hover:bg-cyan-400 text-white font-black py-5 rounded-2xl shadow-sm border-b-[8px] border-cyan-700 active:border-b-0 active:translate-y-[8px] transition-all text-2xl uppercase tracking-wider">Rätta</button>
          ) : (
             <div className={`mt-8 p-6 rounded-[2rem] text-2xl font-black border-4 animate-in zoom-in ${feedback === 'correct' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'}`}>{feedback === 'correct' ? 'Perfekt!' : `Nästan! Ordet var: ${sides.back}`}</div>
          )}
        </form>
      </div>
    </div>
...
function SpeakMode({ words, onBack, ttsLanguage, direction }) {
  ...
  const startListening = () => {
    ...
    recognition.onerror = () => { setIsListening(false); setTranscript("Kunde inte uppfatta. Försök igen!"); };
  ...
  const checkAnswer = (spokenText) => {
    ...
    const cleanCorrect = sides.back.toLowerCase().replace(/[.,!?¿¡]/g, "").trim();
    const isCorrect = cleanSpoken === cleanCorrect;
    setFeedback(isCorrect ? 'correct' : 'incorrect');
    if (isCorrect) setScore(s => s + 1);
    setTimeout(() => {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white p-8 sm:p-12 rounded-[3rem] shadow-xl shadow-lime-100/50 border-4 border-white text-center">
        ...
        <div className="py-12 bg-lime-50 rounded-[2rem] border-4 border-lime-100 mb-10">
           <h2 className="text-5xl sm:text-6xl font-black text-lime-900">{sides.front}</h2>
        </div>
        <div className="flex flex-col items-center justify-center min-h-[180px]">
          {feedback ? (
            <div className={`p-8 rounded-[2rem] w-full text-2xl font-black border-4 animate-in zoom-in ${feedback === 'correct' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
              <p className="text-lg font-bold opacity-70 mb-2 uppercase tracking-wider">Datorn hörde: "{transcript}"</p>
              {feedback === 'correct' ? 'Perfekt uttal!' : `Rätt svar är: ${sides.back}`}
            </div>
          ) : (
            <>
              <button onClick={startListening} disabled={isListening} className={`w-32 h-32 rounded-full flex items-center justify-center transition-all ${isListening ? 'bg-red-500 scale-125 animate-pulse text-white shadow-[0_0_40px_rgba(239,68,68,0.5)] border-4 border-red-300' : 'bg-lime-500 hover:bg-lime-400 border-b-[8px] border-lime-700 active:border-b-0 active:translate-y-[8px] text-white'}`}>
                 <Mic size={56} className={isListening ? 'animate-bounce' : ''} />
              </button>
              <p className={`mt-8 font-black uppercase tracking-widest text-xl ${isListening ? 'text-red-500' : 'text-slate-400'}`}>
                {isListening ? "Prata nu!" : "Klicka på micken"}
              </p>
            </>
          )}
        </div>
  ...
  return (
    <div className="max-w-3xl mx-auto text-center">
      ...
      <div className="bg-white p-8 sm:p-12 rounded-[3rem] shadow-xl shadow-teal-100/50 border-4 border-white">
        <p className="text-teal-600 font-bold uppercase tracking-widest text-sm mb-4">Bygg ordet för</p>
        <h2 className="text-5xl font-black text-slate-800 mb-10">{sides.front}</h2>
        <div className="flex flex-wrap justify-center gap-3 mb-10 min-h-[5rem] p-6 bg-slate-50 rounded-[2rem] border-4 border-dashed border-slate-200">
          {selected.map(item => (
            <button key={item.id} onClick={() => handleDeselect(item)} className="w-16 h-16 flex items-center justify-center bg-teal-500 text-white font-black text-3xl rounded-2xl border-b-[6px] border-teal-700 active:border-b-0 active:translate-y-[6px] transition-all">
              {item.char}
            </button>
          ))}
        </div>
        <div className="flex flex-wrap justify-center gap-3 mb-10">
          {scrambled.map(item => (
            <button key={item.id} onClick={() => handleSelect(item)} className="w-16 h-16 flex items-center justify-center bg-white text-slate-700 font-black text-3xl rounded-2xl border-2 border-slate-200 border-b-[6px] border-b-slate-300 hover:bg-teal-50 hover:border-teal-200 hover:border-b-teal-300 active:border-b-[2px] active:translate-y-[4px] transition-all">
              {item.char}
            </button>
          ))}
        </div>
        {scrambled.length === 0 && !feedback && (
          <button onClick={checkAnswer} className="w-full bg-teal-500 hover:bg-teal-400 text-white font-black py-5 rounded-2xl shadow-sm border-b-[8px] border-teal-700 active:border-b-0 active:translate-y-[8px] transition-all text-2xl uppercase tracking-wider animate-in zoom-in">Rätta</button>
        )}
        {feedback && (
          <div className={`p-6 rounded-[2rem] text-2xl font-black border-4 animate-in zoom-in ${feedback === 'correct' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
             {feedback === 'correct' ? 'Helt rätt!' : `Fel! Rätt svar är: ${sides.back}`}
          </div>
        )}
      </div>
    </div>
...
function HangmanMode({ words, onBack, direction }) {
  ...
  const handleGuess = (letter) => {
    ...
    if (!targetWord.includes(letter)) {
      if (mistakes + 1 >= maxMistakes) { setWordStatus('lost'); setTimeout(nextWord, 3000); }
      setMistakes(m => m + 1);
    } else {
      if (targetWord.split('').every(char => char === ' ' || newGuessed.includes(char))) {
        setWordStatus('won'); setScore(s => s + 1); setTimeout(nextWord, 3000);
      }
    }
  };

  const nextWord = () => {
    if (currentIndex + 1 < words.length) { setCurrentIndex(c => c + 1); setGuessed([]); setMistakes(0); setWordStatus(null); } 
    else setIsFinished(true);
  };
  ...
  return (
    <div className="max-w-3xl mx-auto text-center">
      ...
      <div className="bg-white p-8 sm:p-12 rounded-[3rem] shadow-xl shadow-red-100/50 border-4 border-white">
         <div className="flex justify-between items-center mb-8 bg-red-50 p-4 rounded-2xl border-4 border-red-100">
           <span className="text-red-500 font-bold uppercase tracking-widest text-sm">Gissa ordet för</span>
           <span className="bg-white text-red-600 px-4 py-2 rounded-xl font-black shadow-sm">Försök kvar: {maxMistakes - mistakes}</span>
         </div>
         ...
         <div className="flex justify-center gap-3 sm:gap-4 mb-12 flex-wrap">
            {targetWord.split('').map((char, i) => char === ' ' ? <div key={i} className="w-6 sm:w-10"></div> : (
              <div key={i} className={`w-12 h-16 sm:w-16 sm:h-20 flex items-center justify-center text-4xl font-black rounded-2xl border-b-8 ${guessed.includes(char) || wordStatus === 'lost' ? 'border-slate-300 text-slate-800 bg-slate-100' : 'border-slate-200 text-transparent bg-slate-50'}`}>
                {guessed.includes(char) || wordStatus === 'lost' ? char : '_'}
              </div>
            ))}
         </div>
         ...
         {wordStatus && (
           <div className={`mt-8 p-6 rounded-[2rem] text-2xl font-black border-4 animate-in zoom-in ${wordStatus === 'won' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
              {wordStatus === 'won' ? 'Puh, snyggt räddat!' : 'Ajdå, gubben hängdes!'}
           </div>
         )}
      </div>
    </div>
...
function MatchingMode({ words, onBack }) {
  ...
  useEffect(() => {
    ...
    gameWords.forEach(word => {
      generatedCards.push({ id: `${word.id}-term`, text: word.term, wordId: word.id, type: 'term' });
      generatedCards.push({ id: `${word.id}-trans`, text: word.translation, wordId: word.id, type: 'trans' });
    });
    setCards(generatedCards.sort(() => 0.5 - Math.random()));
    setMatchedPairs([]); setSelectedCards([]); setIsFinished(false);
  }, [words]);
      ...
      if (newSelected[0].wordId === newSelected[1].wordId && newSelected[0].type !== newSelected[1].type) {
        setTimeout(() => {
          setMatchedPairs(prev => {
            const updated = [...prev, newSelected[0].wordId];
            if (updated.length === cards.length / 2) setIsFinished(true);
            return updated;
          });
          setSelectedCards([]);
        }, 500);
  ...
  return (
    <div className="max-w-4xl mx-auto">
       <div className="flex flex-wrap justify-between items-center mb-8 gap-4">
        ...
        <div className="bg-pink-100 text-pink-700 font-black px-6 py-3 rounded-2xl border-4 border-pink-200 shadow-sm">
           Hittade par: {matchedPairs.length} / {cards.length / 2}
        </div>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
        {cards.map(card => {
          ...
          return (
             <button key={card.id} onClick={() => handleCardClick(card)} className={btnClasses} disabled={isMatched} style={{visibility: isMatched ? 'hidden' : 'visible'}}>
                {card.text}
             </button>
          );
        })}
